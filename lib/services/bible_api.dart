...
class BVerse {
  final int verse;
  final String text;
  const BVerse(this.verse, this.text);
}
...
class BibleApi {
  ...
  static const _to = Duration(seconds: 15);
  ...
  Future<BChapter> fetch({
    ...
    if (telugu) return _telugu(book, chapter);
    return _english(book, chapter);
  }

  // ── English — bible-api.com ──────────────────────────────────────────
  Future<BChapter> _english(String book, int chapter) async {
    final url = 'https://bible-api.com/${Uri.encodeComponent('$book $chapter')}?translation=kjv';
    final r = await http.get(Uri.parse(url)).timeout(_to);
    ...
    final vs = (d['verses'] as List).map((v) =>
        BVerse((v['verse'] as num).toInt(), (v['text'] as String).trim())).toList();
    if (vs.isEmpty) throw Exception('No verses found');
    return BChapter(book, chapter, 'KJV', vs);
  }

  // ── Telugu — bolls.life API ──────────────────────────────────────────
  Future<BChapter> _telugu(String book, int chapter) async {
    final id = _teluguId(book);
    if (id == null) throw Exception('Book not found: $book');
    final url = 'https://bolls.life/get-chapter/tel/$id/$chapter/';
    final r = await http.get(Uri.parse(url),
        headers: {'Accept': 'application/json', 'User-Agent': 'ChristConnect/1.0'})
        .timeout(_to);
    if (r.statusCode != 200) {
      throw Exception('Telugu Bible API error (${r.statusCode}).\nCheck internet connection.');
    }
    final body = r.body.trim();
    if (!body.startsWith('[')) {
      throw Exception('Telugu Bible API returned invalid data.\nTry again.');
    }
    final list = json.decode(body) as List;
    if (list.isEmpty) throw Exception('No verses in this chapter.');
    final vs = list.asMap().entries.map((e) {
      final v = e.value as Map<String, dynamic>;
      final num  = (v['verse'] as num?)?.toInt() ?? (e.key + 1);
      String txt = (v['text'] as String? ?? '').trim();
      // Clean HTML entities
      txt = txt
          .replaceAll('&quot;', '"').replaceAll('&amp;', '&')
          .replaceAll('&lt;', '<').replaceAll('&gt;', '>')
          .replaceAll('&#39;', "'").replaceAll(RegExp(r'<[^>]+>'), '');
      return BVerse(num, txt);
